...
export class TaskError extends AppError {
  ...
  readonly traverse = (cb: ((e: Error) => void)) => {
    for (let i = 0; i < this.innerExceptions.length; ++i) {
      const error = NotInitializedError.test(`Error at index ${i},`, this.innerExceptions[i])
